...
def entries(files=None):
    out = []
    for f in files or ['words.js'] + sorted(glob.glob('words/*.js')) + sorted(glob.glob('words/a2/*.js')) + sorted(glob.glob('words/b1/*.js')) + sorted(glob.glob('words/b2/*.js')):
...
def main():
    ...
    for e in es:
        ...
        if 'b2/' in e['file'] and e['level'] != 'B2':
            print(f"WRONG LEVEL {e['cs']}: non-B2 entry in the b2 folder"); bad += 1
